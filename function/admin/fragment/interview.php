            ...
            <ul class="nav nav-pills flex-column mb-auto">
                ...
                <li class="nav-item btn-group">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item active" href="interview.php">Interview</a></li>
        ...
        <div class="container-fluid p-4" style="overflow-x: auto; height: 100vh">
            <div class="row mb-3 align-items-center">
                <div class="col-md-6"> <!-- Kolom pertama untuk judul -->
                    <h1 class="my-4">List of interview results
                    </h1>
                </div>
            </div>
                ...
                <table id="workerTable" class="table table-striped">
                    ...
                    <tbody>
                        ...
                        foreach ($workers as $worker) {
                            $worker_id = $worker['id'];
                            $interview_data = get_worker_interview($worker_id); // Mengambil semua data wawancara
                        
                            // Hanya tampilkan jika ada data wawancara
                            if (!empty($interview_data)) {
                                // Loop melalui data wawancara untuk satu worker
                                foreach ($interview_data as $interview) {
                                    echo '<tr>';
                                    echo "<td>{$counter}</td>";
                                    echo "<td>{$worker['fullname']}</td>"; // Nama
                                    echo "<td>{$worker['position_table_name']}</td>"; // Position
                                    echo "<td>{$interview['interview_date']}</td>"; // Interview Date
                                    echo "<td>{$worker['status_worker_name']}</td>"; // Status
                                    echo "<td>{$interview['overall_recommendation']}</td>"; // Recommendation
                                    echo "<td>{$interview['interviewer']}</td>"; // Interviewer
                        
                                    // Tombol Lihat Detail
                                    echo "<td><a href='././interview_detail.php?id={$interview['interview_id']}' class='btn btn-primary'>Lihat Detail</a></td>";
